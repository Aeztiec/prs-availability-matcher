...
Run with:  python -m tests.test_results
"""
...
check("assists repeat like goals: a10", parse_line("x a10")[1], ["assist"] * 10)
check("so do goals past nine: g12", parse_line("x g12")[1], ["goal"] * 12)
check("but not without limit", parse_line("x a99")[2], ["a99"])
check("yellow and red cards", parse_line("x yc rc")[1], ["yellow", "red"])
check("nosub is no longer a code", parse_line("x nosub")[2], ["nosub"])
